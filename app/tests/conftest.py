...
from ..oauth2 import create_auth_token
from .. import models
...
def test_user2(client):
    user_data = {"email":"testuser1234@gmail.com", "password":"Tarun1995"}
...
def test_user(client):
    user_data = {"email":"testuser@gmail.com", "password":"Tarun1995"}
    res = client.post("/users/", json=user_data)
    assert res.status_code == 201
    new_user = res.json()
    new_user['password'] = user_data['password']
...
@pytest.fixture
def token(test_user):
    return create_auth_token(data={"user_id":test_user["id"]})
...
def test_posts(test_user, test_user2, session):
    ...
    session.commit()
    posts_res = session.query(models.Post).all()
    return posts_res